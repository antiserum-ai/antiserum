...
You point it at a folder. It does a cheap local pass. You read the flags. If a row is poison, you open a pull request that adds a signature to the public feed. The next person never has to find that row by hand.
...
There is no web form. Confirm is a pull request that adds a signature.

See [CONTRIBUTING.md](CONTRIBUTING.md) and [docs/signatures.md](docs/signatures.md). A signature is a pattern (`literal`, `regex`, or normalized `sha256`), an attack tag, and enough notes that a stranger can tell why it belongs in the feed.
