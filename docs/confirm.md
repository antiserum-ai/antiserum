...
| `paraphrase_overweight` | `poison` if a sibling dump/signature landed or the shared 3-gram is specific; else `needs_human` | Shared-phrase families Jaccard misses. A generation template can look the same. |
