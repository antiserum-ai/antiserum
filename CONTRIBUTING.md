...
2. `antiserum judge ./data --receipt receipt.json --out judgments.json`  
   Offline first-pass. `signature_hit` and high-confidence dumps lean poison; strong `hidden_unicode` / `instruction_override` / `mixed_script` hits lean poison; distinctive small-df `pair_trigger` pairs lean poison; weak stat outliers lean junk or false alarm; trigger and label-flip stay `needs_human` unless the evidence is strong. Rubric: [docs/confirm.md](docs/confirm.md).
...
A `false_alarm` that will fire again (a long-but-normal review on `stat_outliers`) belongs in a local `allowlist.jsonl`. `antiserum allowlist add --judgments judgments.json --path ./data` appends the record id and the normalized sha256. Re-running does not duplicate lines. The next scan drops those flags and writes the allowlist path and hash on the receipt. No cloud list.
